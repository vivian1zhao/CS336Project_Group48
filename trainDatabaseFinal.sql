...
foreign key (`schid`) references `trainSchedule` on delete cascade on update cascade,
foreign key (`sid`) references `station` on delete cascade on update cascade);
